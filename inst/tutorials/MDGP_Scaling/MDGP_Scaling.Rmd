...
We will first explore landscape characteristics for random landscape patterns. We will vary the number of classes (c) and the roughness parameter (r) which controls the level of spatial aggregation. For simplicity, the random landscapes will be rasters with equal numbers of rows and columns.
...
```
Change parameters (c) and/or (n) and re-run the code. Every execution generates a new map that can be compared to the previous ones. 

Now vary the roughness factor (r) and observe the effect.
...
The study area was mapped from 2010 and 2013 WorldView 2 (WV2) bi-seasonal data acquired during wet conditions in November 2010 and dry conditions in May 2013.
...
# write relative atbundance to .csv file
...
**How does representativeness affect mean IR?**
...
map_class <- terra::rast(paste0(R.home(),'/library/mdgpTutorial/tutorials/MDGP_Scaling/data_raw/',map_nme))
