...
progressive: true
allow_skip: true
description: "Learn how to use the MDGP-Scaling algorithm of the R package landscapeScaling. The first section of the tutorial introduces the necessity and complexity of categorical data scaling theory. A new method that considers information retention and representativeness of the scaled classes when scaling categorical raster data is introduced. Several exercises demonstrate the theoratical concepts"
...
remotes::install_github("gannd/landscapeScaling", force=TRUE)
...
The current version of {landscapeScaling} was installed from [GitHub](https://github.com/) with remotes (remotes::install_github("gannd/landscapeScaling") during the installation of this tutorial.
...
c <- 4   # number of classes: 1 more than classes ; first break point 0 and last break point 1 are not included
r <- 0.04   # roughness
...
nlm_cat[is.na(nlm_cat)] <- 0

# assign class names to the three raster values 
levels(nlm_cat) <- data.frame(value=c(0:(c-2)),lc=c(LETTERS[1:(c-1)])) 
# -----------------------------------------------
# plot the categorized landscape
x11();terra::plot(nlm_cat,col=col12,mar=c(1.5,1.5,1.5,8),
            main=paste0((n-1)^2,' Cell Landscape of ',c-1,' Classes and Roughness ',r),
            cex.main = 1.2,
...
We will first generate the landscape that will then be scaled with scale factors 3 and 5. The scaling result will then be converted to rasters of scaled classes and their associated cell-level $IR_{c}$.
...
r <-    # roughness
c <-    # number of classes - remember to add 1 more
