...
The scaling output includes scaled categorical raster maps with mixed classes, a corresponding continuous raster with compositional information retention calculations for each scaled grid cell, class-specific and at the landscape-level mean and standard deviation of information retention.
...
**Verify where the working directory has been set to. this location will change in every run of every chunk. Therefore we will set our working directory explicitly in every chunk where we need to read data for subsequent exercises.**
\t
...
When we observe a system at different spatial scales and hierarchical levels different patterns emerge. Temporal analysis of system state changes rely on the tractability of spatial patterns so that they compare across space and time. Often the scales of data sources vary over time and comparability is lost. With the vast expansion of ever increasing spatial and temporal resolution of remotely sensed data, the need for good scaling practices is increasing.

### Why Do We Need Spatial Scaling? 

To be able to apply models that are informed by small-scale data at larger scales (larger extent and/or grain size) data samples (point or spatially exhaustive data) are summarized or aggregated. 

Many ecological models rely on information patterns of distribution and dynamics of *primary producers* (i.e specie or functional types of vegetation). Three examples that demonstrate the importance of scale appropriate information scaling of vegetation information for ecological process models. 

#### Earth System Models 
Estimating biophysical variables from remotely sensed data. Difficulties produced by spatial heterogeneity on the reliable estimation of biophysical variables using remotely sensed data have been identified and described for a suite of parameters and applications (Lu 2006). 
...
Estimation of these variables from Moderate Resolution Imaging Spectroradiometer (MODIS), which has high temporal resolution (daily) but coarse spatial resolution (500 m), relies on land-cover knowledge of each pixel (Steltzer and Welker 2006; Zhao et al. 2016).

Modeling GPP for wetlands that display high heterogeneity of land cover relative to the coarse resolution of MODIS is difficult (Feagin et al. 2020).

LAI and FPAR are sensitive to land-cover information (Lotsch et al. 2003), and the heterogeneity of vegetation types within a pixel affects LAI estimates in a nonlinear fashion (Garrigues et al. 2006).

LAI errors at a coarse resolution are inversely related to the proportion of the dominant land cover in a pixel and large errors are introduced when the woody component makes up only a small proportion of otherwise non-woody pixels (Tian et al. 2002).
...
## generate the landscape with 81 grid cells and 3 classes wirh r = 0.01 or 0.95
...
print('ADD THE PROPORTION VALUES IN THE CORRESPONDING VECTOR IN THE NEXT CHUNK BEOFRE RUNNING THE NEXT ITERATION!', col='red')
```
...
**What effect did heterogeneity of the landscape have on class percentage differences and compositional IR when scaling at a scale factor of 3?**   

```{r, questions-1-response, exercise=TRUE}
print('Answer: ')
```
----
...
Classification of plant communities on the basis of quantitative data and analytical methods is common in vegetation ecology. Quantitative methods for vegetation classification go back to Braun Blanquet (citation) More recent methods and ... are covered in O. Wildi (), DeCaseres (), .... 
...
Sampling methods that consider sampling design to assure **representativeness** of the classification across the landscape are necessary.
...
Scaling entire landscapes using MDGP-Scaling generates frequency distributions of mixed classes that depend on:
...
terra::writeRaster(nlm_cat,paste0('./data/nlm_',n-1,'.tif'),overwrite=TRUE)
```

The landscape was written to the "./data" folder as a ".tif" file. It will be loaded in all subsequent chunks.
...
# read the landscape
map <- terra::rast('./data/nlm_[X].tif')   # replace X wit the resolution of the map to be used
classField <- 'lc'
...
saveRDS(mdgp_result,file=paste0('./data/',prfx,'_sf',sf,'_prts',prts,'.rds'))
```
...
mdgp_result <- readRDS('./data/X.rds') # replace X.rds with the name of the scaling results file
...
setwd(wd)
print(read.csv('./data_raw/plant_communities_13_classes_LUT.csv'))
```
...
print(read.csv('./data_raw/plant_communities_RCL.csv'))

print('The reclassified map LUT: ')
print(read.csv('./data_raw/plant_communities_5_classes_LUT.csv'))
```
----

MAYBE: Load and print the map with leaflet.
...
# load map 
veg_map <- terra::rast('./data_raw/plant_communities_wv2.tif')
lut13 <- read.csv('./data_raw/plant_communities_13_classes_LUT.csv')
levels(veg_map) <- lut13
print(veg_map)

# load the reclassification table
rcl <- read.csv('./data_raw/plant_communities_RCL.csv')

# reclassify the map
veg_map_rcl <- terra::classify(veg_map,rcl[,c(1,3)])
lut5 <- read.csv('./data_raw/plant_communities_5_classes_LUT.csv')
levels(veg_map_rcl) <- lut5

terra::writeRaster(veg_map_rcl,'./data_raw/plant_communities_5_classes.tif',overwrite=TRUE)
...
# load map 
veg_map <- terra::rast('./data_raw/plant_communities_5_classes.tif')
classField <- 'class'
...
write.csv(rel_abund,file=paste0('./data/pc5_sf',sf,'_rel_abund.csv'),row.names = FALSE)
```
...
rel_abund <- read.csv('./data/pc5_sf15_rel_abund.csv')
...
# write the scaled data frame to .rds file
saveRDS(mdgp_result,file=paste0('./data/',prfx,'prts',prts,'_rpr',rpr,'_mono',mono,'.rds'))
# -----------------------------------------------
# convert classified points and location-specific information retention to raster
...
terra::writeRaster(mdgp_raster[[1]],file=paste0('./data/',prfx,'prts',prts,'_rpr',rpr,'_mono',mono,'_class.tif'))
```
...
ls <- terra::rast('./data_raw/L5TM_20111110_atmcor_sub.tif')
...
```{r, questions-5-1-response, exercise=TRUE}
print('Answers: ')
```
...
# load spectral Landsat data
spc.rst <- terra::rast(paste0('./data_raw/',ls_nme))

# load scaled map and convert to vector
map_class <- terra::rast(paste0('./data/',map_nme))
...
write.table(mtxCnf.cv$byClass,paste0('.data/',prfx,'_mtxCnf_stats.csv'))
write.table(mtxCnf.cv$table,paste0('.data/',prfx,'_mtxCnf.csv'))
# -----------------------------------------------
...
# load spectral Landsat data
spc.dat <- terra::rast('./data_raw/L5TM_20111110_atmcor_sub.tif')

# load scaled map and associated information retention layer
map_scl <- terra::rast('./data/[SCALED_MAP_NAME].tif')

# load information retention raster and clacualte mean
map_infRtn <- raster(irC)
print(infRtnRL <- cellStats(map_infRtn, 'mean'))


names(map_scl) <- 'cls'
values(map_scl)[values(map_scl) == 0] = NA
map.pnt <- rasterToPoints(map_scl, spatial=T)

# join attribute data - class labels
map.pnt <- merge(map.pnt,map_scl@data@attributes[[1]],by.x='cls',by.y='ID')	

# extract signature and join to class info
trnDat <- as.data.frame(terra::extract(spc.rst, map.pnt, sp=TRUE))
head(trnDat)

# number or classes in realized landscape grid 
clsNumRL <- length(unique(trnDat$category))


#------------------------------------------------

# EVALUATE SIGNATURE -- CROSS-VALIDATED RANDOM FOREST MODEL

mdlFit <- caret:::train(trnDat[,3:(nlayers(spc.rst)+2)],as.character(trnDat$category),trControl = trCtrl, metric='Accuracy', method='rf', tuneGrid=gridTune, ntree=nTree, importance=TRUE) #, trControl = trCtrl, metric=mtr, method=clfMeth, tuneGrid=gridTune, ntree=numTrees, importance=TRUE))
#print(mdlFit)
```
